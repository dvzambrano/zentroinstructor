    ...
    return {
        init : function(App) {
                            ...
                            listeners: {
                                click: function() {
                                    ...
                                    if (config.app_logueduserdata){
                                        ...
                                        if(config.app_logueduserdata.profile && config.app_logueduserdata.profile.schedule){                                        
                                            if(config.app_logueduserdata.profile.schedule.sesions){
                                                var array = Ext.decode(config.app_logueduserdata.profile.schedule.sesions);
                                                if (array){
                                            ...
                                            if(config.app_logueduserdata.profile.schedule.exceptions){
                                                var array = Ext.decode(config.app_logueduserdata.profile.schedule.exceptions);
                                                if (array){
                                                    window['UserApp'].scheduleDetailsTabPanel.exceptionDatesGridPanel.getStore().removeAll();
                                                    for(var i = 0; i < array.length; i++){
                                                        window['UserApp'].scheduleDetailsTabPanel.exceptionDatesGridPanel.getStore().insert(window['UserApp'].scheduleDetailsTabPanel.exceptionDatesGridPanel.getStore().getCount(), new Ext.data.Record({
            ...
            this.mainmenuTreePanel = new Ext.tree.TreePanel({
                ...
                iconCls: Ext.ux.Icon('application-x-theme', 'myicons'),
                rootVisible: false,
                border: false,
                lines: true,
                //singleExpand: true,
            ...
            this.messagePanel = new Ext.Panel({
                ...
                tools: [{
                    ...
                    qtip: bundle.getMsg('app.languaje.refresh.label'),
                    handler: function(){
                        App.statisticsStore.loadforced = true;
                ...
                items: [new Ext.DataView(Ext.apply({
                    ...
                    listeners: {
                        render: function() {
                            if(config.app_notificationrefreshperiod && config.app_notificationrefreshperiod > 0)
                                Ext.TaskMgr.start({
                                    run: function(){
                                        App.statisticsStore.load();
                                    },
                                    interval: 1000 * config.app_notificationrefreshperiod
                                });
                            else
                                App.statisticsStore.load();
                        }
                    ...
                    tpl: '<div style="padding:5px"><tpl for=".">' +
                    '<table width="100%">' +
                    '<tr style="font-size:{size}px;">' +
                    '<td width="80%"><b>{bundlename}</b>:</td>' +
                    '<td width="20%"><tpl if="flag != \'542382\'"><a href="#" onclick="javascript:App.setView(&#39;{name}&#39;)" style="text-decoration: none;"></tpl><div class="ext-cal-evt ext-cal-evr" align="center" style="background-color:#{flag};">&nbsp;{amount}&nbsp;<tpl if="flag == \'542382\'"><i class="ext-cal-ic ext-cal-ic-rem"> </i></tpl>&nbsp;</div><tpl if="flag != \'542382\'"></a></tpl></td>' +
                    '</tr>' +
                    '</table>' +
                    '</tpl></div>',
        ...
        setView : function(param){   
            var initContext = function(callback){
                var m = getModule('Plan');                                
                var menu = Ext.getCmp('principal-menu-item-'+m.id);
                if(menu){
                    menu.toggle(true);
                    Ext.defer(callback, 1000, this);
                }
            };
                             
            var showView = function(){
                if(window['PlanApp'] && window['PlanApp'].cardPanel){
                    window['PlanApp'].taskGridPanel.setTitle(bundle.getMsg(param));
                    if(window['PlanApp'].cardPanel.getLayout().setActiveItem){ 
                        window['PlanApp'].taskGridPanel.getStore().baseParams.person = config.app_logueduserdata.personid;
                        window['PlanApp'].taskGridPanel.getStore().baseParams.time = param.replace('task.', '').replace('.title', '');
                        
                        if(window['PlanApp'].cardPanel.getLayout().activeItem && window['PlanApp'].cardPanel.getLayout().activeItem.id == 'gridPanelTask')
                            window['PlanApp'].cardPanel.getLayout().activeItem.fireEvent('activate',  window['PlanApp'].cardPanel.getLayout().activeItem);
                        else
                            window['PlanApp'].cardPanel.getLayout().setActiveItem(1);
                        
                    }
                    else
                        Ext.defer(showView, 1000, this);
                }
                else
                    initContext(showView);
            };
