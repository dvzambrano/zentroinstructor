...
class TasktypeTable extends Doctrine_Table {
    ...
    const table = 'Tasktype';
    const akfield = 'code';

    public static function getAllPaged($start, $limit, $filters, $simple = false) {
        $select = 't.icon as customicon, "" as icon, (SELECT COUNT(q.id) FROM Task q WHERE q.tasktypeid = t.id)<1 as deleteable';
    ...
    public static function getByParent($filters = array(), $checkeable = false) {
        $select = 'p.*, (SELECT COUNT(q.id) FROM Task q WHERE q.tasktypeid = t.id)<1 as deleteable';
        return BaseTable::getByParent(self::table, $filters, $checkeable, $select, array('t.Tasktype p'));
    }
    ...
    public static function getRebuilded($array = array()) {
        ...
        if (!$tasktype && $array['code'] != '')
            $tasktype = self::findByAK($array['code']);

        if (!$tasktype && $array['id'] > 0) {
            $tasktype = new Tasktype();
            $tasktype->fromArray($array);
            $parent = Doctrine::getTable('Tasktype')->getRebuilded($array['Tasktype']);
            if ($parent)
                $tasktype->setTasktype($parent);
            $tasktype->save();
        }
